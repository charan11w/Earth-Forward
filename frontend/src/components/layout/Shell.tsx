...
export default function Shell({children}:{children:React.ReactNode}){
  ...
  const crumbs=parts.map((p,i)=>({label:labels[p]||({users:'User details',routes:'Route details',stops:'Stop details',pickups:'Pickup details'} as Record<string,string>)[parts[i-1]]||'Details',path:'/'+parts.slice(0,i+1).join('/')}));
